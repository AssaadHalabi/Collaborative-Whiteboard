...
export const handleCanvasObjectModified = ({
  ...
  if (target?.type == "activeSelection") {
    // const targets : Array<fabric.Object> = (target as any)._objects;
    // targets.forEach(t =>{
    //   syncShapeInStorage(t);
    // })
  } else {
...
export const handleCanvasSelectionCreated = ({
  ...
  if (selectedElements.length === 1) {
    ...
    const attributes = selectedElements.map((element) => {
      const scaledWidth = element?.scaleX
        ? element?.width! * element?.scaleX
        : element?.width;

      const scaledHeight = element?.scaleY
    ...
    // Set attributes for the first selected element for now
    setElementAttributes(attributes[0]);
  }
};
  ...
  if (target?.type === "activeSelection") {
    ...
    activeSelection.getObjects().forEach((object) => {
      const scaledWidth = object?.scaleX
        ...
        : object?.width;
      const scaledHeight = object?.scaleY
...
export const handleResize = ({ canvas }: { canvas: fabric.Canvas | null }) => {
  ...
  const minWidth = 300; // Minimum width of the canvas
  const minHeight = 300; // Minimum height of the canvas
